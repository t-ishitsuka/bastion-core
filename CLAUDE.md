...
5. **軽量化** - 外部依存を最小化、標準ライブラリを優先

## 技術スタック

- **言語**: Go 1.22+
- **CLI フレームワーク**: spf13/cobra v1.8.0
- **設定ファイル**: BurntSushi/toml v1.3.2
- **ファイル監視**: fsnotify（標準的な選択）
- **並列実行**: tmux + git worktree

外部依存は最小限に抑え、標準ライブラリを優先して使用。
...
- 新しいコードを書く際は必ずテストを書く
- コードを変更したら必ず golangci-lint を実行する
- テーブル駆動テストを推奨
